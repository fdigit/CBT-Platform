  ...
  function middleware(req) {
    ...
    if (pathname.startsWith('/admin')) {
      if (userRole !== Role.SUPER_ADMIN) {
        return NextResponse.redirect(new URL('/auth/signin?error=insufficient-permissions', req.url))
      }
    }
    ...
    if (pathname.startsWith('/school')) {
      ...
      if (!schoolId) {
        return NextResponse.redirect(new URL('/auth/signin?error=no-school-assigned', req.url))
      }
    }
