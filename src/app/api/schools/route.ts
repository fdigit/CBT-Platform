  ...
  try {
    ...
    if (action) {
      ...
      if (!['PENDING', 'APPROVED', 'SUSPENDED', 'REJECTED'].includes(status)) {
        return NextResponse.json(
          { message: 'Invalid status. Must be PENDING, APPROVED, SUSPENDED, or REJECTED' },
          { status: 400 }
        )
      }
    ...
    if (updatePayload.status && ['APPROVED', 'REJECTED', 'SUSPENDED'].includes(updatePayload.status)) {
      try {
