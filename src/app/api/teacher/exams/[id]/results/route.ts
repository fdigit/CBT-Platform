...
export async function GET(
  ...
  try {
    ...
    const averageScore = totalStudents > 0 ? 
      results.reduce((sum, r) => sum + r.score, 0) / totalStudents : 0
    ...
    const answersByStudent = studentAnswers.reduce((acc, answer) => {
    ...
    const formattedResults = results.map(result => {
      const studentAttempts = attempts.filter(a => a.studentId === result.studentId)
      const latestAttempt = studentAttempts.sort((a, b) => 
        new Date(b.startedAt).getTime() - new Date(a.startedAt).getTime()
      )[0]

      const studentAnswersData = answersByStudent[result.studentId]?.answers || []
      const objectiveScore = studentAnswersData
        .filter((a: any) => ['MCQ', 'TRUE_FALSE'].includes(a.questionType) && a.pointsAwarded !== null)
        .reduce((sum: number, a: any) => sum + (a.pointsAwarded || 0), 0)
      
      const subjectiveScore = studentAnswersData
        .filter((a: any) => ['ESSAY', 'SHORT_ANSWER'].includes(a.questionType) && a.pointsAwarded !== null)
        .reduce((sum: number, a: any) => sum + (a.pointsAwarded || 0), 0)

      const unansweredQuestions = studentAnswersData.filter((a: any) => 
      ...
      return {
        ...
        studentClass: result.student.class ? 
          `${result.student.class.name} ${result.student.class.section || ''}` : 'N/A',
        regNumber: result.student.regNumber,
        score: result.score,
        totalMarks: exam.totalMarks || 0,
        percentage: exam.totalMarks ? (result.score / exam.totalMarks) * 100 : 0,
        passed: exam.passingMarks ? result.score >= exam.passingMarks : result.score > 0,
    ...
    return NextResponse.json({
      ...
      statistics: {
        ...
        highestScore,
        lowestScore,
        averagePercentage: exam.totalMarks ? (averageScore / exam.totalMarks) * 100 : 0
      },
