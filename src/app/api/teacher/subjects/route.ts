  ...
  try {
      ...
      teacherSubjects.map(async (ts) => {
        ...
        const recentActivities = [
          ...
          },
          {
            ...
            class: classes[Math.floor(Math.random() * classes.length)]?.displayName || 'No classes',
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        totalClasses: subjectsWithClasses.reduce((sum, subj) => sum + subj.totalClasses, 0),
        totalStudents: subjectsWithClasses.reduce((sum, subj) => sum + subj.totalStudents, 0),
        averageScore: subjectsWithClasses.length > 0 
          ? Math.round(subjectsWithClasses.reduce((sum, subj) => sum + subj.averageScore, 0) / subjectsWithClasses.length)
          : 0
      }
    })
  } catch (error) {
