...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let i = 5; i >= 0; i--) { // Reduced from 12 to 6 months
    ...
    const reports = {
      ...
      insights: {
        ...
        growthTrend: userGrowthData.length > 1 ? 
          ((userGrowthData[userGrowthData.length - 1].users - userGrowthData[userGrowthData.length - 2].users) / Math.max(userGrowthData[userGrowthData.length - 2].users, 1) * 100) : 0,
        alerts: []
      }
    }
