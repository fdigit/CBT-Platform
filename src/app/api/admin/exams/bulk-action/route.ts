...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!examIds || !Array.isArray(examIds) || examIds.length === 0) {
      return NextResponse.json({ message: 'Invalid exam IDs' }, { status: 400 })
    }
    ...
    if (exams.length !== examIds.length) {
      return NextResponse.json({ message: 'Some exams not found' }, { status: 404 })
    }
    ...
    if (action === 'approve') {
      ...
      const examsWithResponses = exams.filter(exam => 
        exam._count.results > 0 || exam._count.answers > 0
      )

      if (examsWithResponses.length > 0) {
        return NextResponse.json({
          message: 'Some exams cannot be deleted as they have student responses',
