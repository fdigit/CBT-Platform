...
export async function DELETE(
  ...
  try {
    ...
    if (existingClass._count.students > 0) {
      return NextResponse.json(
        { message: 'Cannot delete class with enrolled students. Please move students to another class first.' },
        { status: 400 }
      )
    }
    ...
    if (existingClass._count.exams > 0) {
      return NextResponse.json(
        { message: 'Cannot delete class with existing exams. Please remove or reassign exams first.' },
        { status: 400 }
      )
    }
