...
export async function GET(request: NextRequest) {
  try {
    ...
    const workloadDistribution = workloadData.map(teacher => {
      const classCount = teacher.classes.length
      const studentCount = teacher.classes.reduce((sum, cls) => sum + cls.students.length, 0)
      
      // Calculate workload score (simple algorithm)
      const workloadScore = Math.min(100, (classCount * 20) + (studentCount * 0.5))
