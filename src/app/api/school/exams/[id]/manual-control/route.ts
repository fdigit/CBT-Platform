...
export async function POST(
  ...
  try {
    ...
    // Validate action
    if (
      !['make_live', 'make_completed', 'toggle_manual_control'].includes(action)
    ) {
    ...
    switch (action) {
      case 'make_live':
        ...
        const hasActiveAttempts = existingExam.attempts.some(
          attempt =>
        ...
        if (hasActiveAttempts) {
          return NextResponse.json(
            {
              error:
                'Cannot make exam live - students have already started taking this exam',
            },
            { status: 400 }
          );
        }
