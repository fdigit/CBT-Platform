...
export async function POST(request: NextRequest) {
  try {
    ...
    if (subjects.length !== validatedData.subjectIds.length) {
      return NextResponse.json({ message: 'One or more subjects not found' }, { status: 404 })
    }
    ...
    const unassignedSubjects = validatedData.subjectIds.filter(id => !assignedSubjectIds.includes(id))
    ...
    const newSubjectIds = validatedData.subjectIds.filter(id => !existingSubjectIds.includes(id))
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!classItem || !teacher || !subject) {
      return NextResponse.json({ message: 'Class, teacher, or subject not found' }, { status: 404 })
    }
    ...
    return NextResponse.json({ message: 'Class assignment removed successfully' })
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { message: 'Validation error', errors: error.errors },
