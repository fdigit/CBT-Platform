...
export function AddClassModal({ isOpen, onClose, onClassAdded, teachers }: AddClassModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new class and assign teachers to manage students effectively.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            <div className="border rounded-lg p-4 max-h-48 overflow-y-auto">
              {availableTeachers.length === 0 ? (
                <p className="text-sm text-gray-500">No active teachers available</p>
              ) : (
                <div className="space-y-2">
                  {availableTeachers.map(teacher => (
                    <div key={teacher.id} className="flex items-center space-x-2">
                      ...
                      >
                        ...
                        <span className="text-gray-500 ml-2">({teacher.employeeId})</span>
                        {teacher.specialization && (
                          <span className="text-blue-600 ml-2">• {teacher.specialization}</span>
                        )}
                      </label>
                    </div>
                  ))}
            ...
            {selectedTeachers.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2">
                {selectedTeachers.map(teacherId => {
                  ...
                  return teacher ? (
                    <Badge key={teacherId} variant="outline" className="flex items-center gap-1">
                      {teacher.name}
                      <X 
