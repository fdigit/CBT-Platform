...
export function AssignClassesForm({ 
  ...
  const fetchTeacherSubjects = async (teacherId: string) => {
    ...
    try {
      const response = await fetch(`/api/school/teachers/${teacherId}/subjects`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedClassId || !selectedTeacherId || selectedSubjectIds.length === 0) {
      toast({
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to assign subjects to class',
        variant: 'destructive'
      })
    } finally {
      setIsLoading(false)
        ...
        {selectedTeacherId && (
          <div className="space-y-3">
            <Label>
              ...
              {loadingSubjects && (
                <span className="ml-2 text-sm text-blue-600">Loading subjects...</span>
              )}
            </Label>
