...
export function AssignClassesModal({
  ...
  const fetchClassAssignmentData = async () => {
    ...
    try {
      const response = await fetch(
        `/api/school/teachers/${teacher.id}/assign-classes`
      );
      if (response.ok) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: 'Error',
          description:
            errorData.message || 'Failed to update class assignments',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const filteredClasses =
    classData?.availableClasses.filter(
      cls =>
          ...
          <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
            ...
            {selectedCount > 0 && (
              <Badge variant="secondary">
                {selectedCount > currentlyAssignedCount
                  ? `+${selectedCount - currentlyAssignedCount}`
                  : selectedCount < currentlyAssignedCount
                    ? `-${currentlyAssignedCount - selectedCount}`
                    : 'No change'}
              </Badge>
            )}
          </div>
